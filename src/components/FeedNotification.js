    ...
    render() {
        ...
        return (
            ...
            <View style={[styles.container]}>
                <Text style={[styles.text]}>You have {this.props.messages.length} new {messages.length > 1 ? labelPlural : labelSingular} drag down to refresh</Text>
            </View>
            : null
        );
    }
...
class FeedNotificationInner extends React.Component<PropsInner, State> {
    ...
    async componentDidMount() {
        ...
        let subscription = feed.subscribe(((data) => {
            ...
            console.log(`now listening to changes in realtime ${this.props.feedGroup}:${this.props.user.id}`);
        }, (err) => {
            console.error(err);
        });
    ...
    render() {
        ...
        return (
            <Child {...this.props} messages={this.state.messages} />
        );
    }
}
